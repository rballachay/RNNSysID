...
class Signal:
    ...
    def y_map_function(self,iterator):
        ...
        for j in range(0,self.outDim):
            ...
            denTemp = [[1.,-self.tauArray[iterator,self.outDim*j+i]] for i in range(self.inDim)]
    ...
    def sys_simulation(self,stdev=5,KpRange=[.5,.99],tauRange=[.5,.99],thetaRange=[1,10],order=False):
        """
        ...
        self.thetaRange = thetaRange; self.order = order
    ...
    def system_validation(self,KpRange=[.5,.99],tauRange=[.5,.99],thetaRange=[1,10],order=False):
        ...
        uArray,yArray,taus,kps,thetas,train,test = self.sys_simulation(stdev=self.stdev,KpRange=KpRange,tauRange=tauRange,thetaRange=thetaRange,order=order)
    ...
    def y_map_higher_function(self,iterator):
        ...
        # Create first PRBS outside of loop
        u = self.uArray[iterator,:,:]
        a,b = self.KpArray.shape
        self.kpFinal = np.zeros((a,b,self.order))
        self.tauFinal = np.zeros((a,b,self.order+1))
