...
class Model:    
    ...
    def load_model(self,sig,directory=False,check=False):
        ...
        self.length,self.width,self.height=sig.xData['kp'].shape
        if not(directory):
        ...
        if stepResponse and sig.inDim==1:
            ...
            for (idx,parameter) in enumerate(['kp','tau','theta']):
                ...
                if parameter=='theta':
                    ...
                    ax.plot(np.linspace(0.5,1),np.linspace(0.5,1),'--r',label = r2)
                ax.legend()
    ...
    def step_response_validation(self,sig,predDict,errDict):
        ...
        sses = np.zeros(99)
        for i in range(0,len(predDict['kp'])):
            if i<10:
                plt.figure(i+100,dpi=200)
            for j in range(0,100):
                if j<99:
                    gauss_kp = np.random.normal(predDict['kp'][i],errDict['kp'][i]/2)
                    gauss_tau = np.random.normal(predDict['tau'][i],errDict['tau'][i]/2)
                    gauss_theta = abs(int(np.ceil(np.random.normal(predDict['theta'][i],errDict['theta'][i]/2))))
                    if gauss_theta==0:
                        gauss_theta=1
                    numTemp = [gauss_kp]
                    denTemp = [1.,-gauss_tau]
                    ...
                    temp[gauss_theta:] = resp[:-gauss_theta]
                    if i<10:
                    ...
                    numTemp = [predDict['kp'][i]]
                    denTemp = [1.,-predDict['tau'][i]]
                    sys = control.tf(numTemp,denTemp,1)
                    _,resp,_ = control.forced_response(sys,U=sig.test_sequence,T=np.linspace(0,sig.length-1,sig.length))
                    temp = np.zeros_like(resp)
                    theta=abs(int(np.ceil(predDict['theta'][i])))
                    if theta==0:
                        theta=1
        ...
        df['Kp'] = sig.yData['kp'].flatten()
        df['tau'] = sig.yData['tau'].flatten()
        df['theta'] = sig.yData['theta'].flatten()
        df['Kp Pred'] = predDict['kp']
        df['tau Pred'] = predDict['tau']
        df['theta Pred'] = predDict['theta']
        df['Kp Std'] = errDict['kp']
        df['tau Std'] = errDict['tau']
        df['theta Std'] = errDict['theta']
    ...
    def posterior_mean_field(kernel_size, bias_size=0, dtype=None):
      ...
      return tf.keras.Sequential([
          ...
          # Changed from 1e-5 to 1e-3 and 1*tf.nn.softplus to 10
