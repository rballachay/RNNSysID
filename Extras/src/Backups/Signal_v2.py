...
class Signal:
    ...
    def stretch_MIMO(self,name):
        ...
        self.xDataMat = np.full((a*self.outDim*self.inDim,b),0.)
        self.yDataMat = np.full((a*self.outDim*self.inDim,1),0.)
        if name=='kp':
            for i in range(0,self.inDim):
                for j in range(0,self.outDim):
                    dim = self.inDim
                    self.yDataMat[a*(dim*i+j):a*((dim*i+j)+1),:] = kps[:,(dim*i+j):((dim*i+j)+1)]
                    self.xDataMat[a*(dim*i+j):a*(dim*i+j+1),:] = yArray[:,:,i]*uArray[:,:,j]
                    
        elif name=='tau':
            for i in range(0,self.inDim):
                for j in range(0,self.outDim):
                    dim = self.inDim
                    self.yDataMat[a*(dim*i+j):a*((dim*i+j)+1),:] = taus[:,(dim*i+j):((dim*i+j)+1)]
                    self.xDataMat[a*(dim*i+j):a*(dim*i+j+1),:] = yArray[:,:,i] - uArray[:,:,j]
            ...
            for i in range(0,self.inDim):
                for j in range(0,self.outDim):
                    dim = self.inDim
                    self.yDataMat[a*(dim*i+j):a*((dim*i+j)+1),:] = thetas[:,(dim*i+j):((dim*i+j)+1)]
                    self.xDataMat[a*(dim*i+j):a*(dim*i+j+1),:] = yArray[:,:,i] - uArray[:,:,j]
