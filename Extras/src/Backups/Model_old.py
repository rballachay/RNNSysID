...
class Model:    
    ...
    def train_MIMO(self,sig=False,plotLoss=True,plotVal=True,epochs=50,saveModel=True,checkpoint=False):
        ...
        for (k,parameter) in enumerate(parameters):
            ...
            if sig.inDim==2 and sig.inDim==2:
                ...
                print("This size of model doesn't exist. Please create a new %i x %i RNN model." %(sig.inDim,sig.outDim))
  ...
  # Specify the prior over `keras.layers.Dense` `kernel` and `bias`.
    ...
    def __FOPTD_probabilistic(self): 
        "Probabilistic model for SISO data"
        # GRU took 135 s/step to train (final val coeff. determination = 0.9718, coef. determination = 0.9727 )
        negloglik = lambda y, rv_y: -rv_y.log_prob(y)
        model = tf.keras.Sequential([  
        tf.keras.layers.GRU(50,input_shape=(self.nstep,1)),
