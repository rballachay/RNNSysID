...
class Signal:
    ...
    def sys_simulation(self,stdev=5,KpRange=[1,10],tauRange=[1,10],thetaRange=[1,10]):
        ...
        self.uArray = np.zeros((self.numTrials,int(100*self.inDim),self.inDim))
        for trial in range(0,numTrials):
            seq = self.PRBS()
            for dim in range(0,self.inDim):
                dimseq = np.zeros(int(100*self.inDim))
                dimseq[dim*100:(dim*100+100)]=seq
