...
class Model:    
    ...
    def load_and_train(self,sig,epochs=50,probabilistic=True,saveModel=True,batchSize=16,plotLoss=False,plotVal=False):
        ...
        self.train_model(sig, saveModel=saveModel,epochs=epochs,checkpoint=False,batchSize=batchSize)
    ...
    def train_model(self,sig=False,plotLoss=True,plotVal=True,epochs=50,saveModel=True,
        ...
        for (k,parameter) in enumerate(parameters):
            if sig.inDim==2 and parameter=='kp':
                continue
    ...
    def predict_system(self,sig,plotPredict=True,savePredict=False,probabilityCall=False):
        ...
        predDict = dict()
        errDict = dict()
        predDict['kp'] = np.array(kp_yhat.mean())   
        errDict['kp'] = np.array(2*kp_yhat.stddev())
        
        predDict['tau'] = np.array(tau_yhat.mean())    
        errDict['tau'] = np.array(2*tau_yhat.stddev())  
        
        predDict['theta'] = np.array(theta_yhat.mean())    
        errDict['theta'] = np.array(2*theta_yhat.stddev())  
  ...
  # Specify the prior over `keras.layers.Dense` `kernel` and `bias`.
    ...
    def mutable_model(self,x_train,y_train):
        ...
        tf.keras.layers.Dense(int(10),activation='linear'),
        tfp.layers.DenseVariational(2,Model.posterior_mean_field,Model.prior_trainable,activation='linear',kl_weight=1/x_train.shape[0]),
